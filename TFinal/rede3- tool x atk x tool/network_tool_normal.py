...
edges = pd.read_csv("./tool_attacks_edges.csv")
nodes = pd.read_csv("./tool_attacks_nodes.csv")
...
for _, row in edges.iterrows():
    ...
    try:
        attack_categories = (
            json.loads(row["category"]) if not pd.isna(row["category"]) else []
        )
    except Exception:
        attack_categories = []
...
plot_degree_distribution_complementar(
    ...
    titles=["Tool-Tool"],
    output_path="./",
    output_file_name="tool_degree_distribution.png",
    show=False,
)

# Calcular posições dos nós (layout)
pos = nx.spring_layout(G, seed=42)


# Exporte uma imagem da rede
...
plt.axis("off")
plt.savefig("tool_network_graph.png", format="PNG")
plt.close()
print(f"Grafo exportado usando Matplotlib: tool_network_graph.png")
